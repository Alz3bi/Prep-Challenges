...
const objectCounter = (obj) => {
    const counter = obj.reduce((accumulator) => accumulator += 1, 0)
    return counter
...
const statistics = (obj) => {
    let res = {}
    const assign = obj.reduce((accumulator, currentValue, index) => {
        if (index === 1) {
            res[accumulator.votes_To] = 1;
        }
        if (res.hasOwnProperty(currentValue.votes_To)) {
            res[currentValue.votes_To] += 1;
        }else{
            res[currentValue.votes_To] = 1;
        }
    })
