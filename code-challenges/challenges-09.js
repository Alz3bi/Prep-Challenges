...
const employeesBonus = (arr) => {
    ...
    arr.forEach((element) => {
        if (element.workHours > 8) {
            let salary = Number(element.salary.slice(0, 4));
            salary += 100;
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    let combination = mouseArray[0]+keyBoardArray[0];
    keyBoardArray.forEach(element => {
        mouseArray.forEach(insideElement => {
            let total = element + insideElement;
            if (total > combination && total <= budget) {
                combination = total;
            }
        });
    });
